...
Six jobs run on every PR and push to `main`, plus a weekly cron schedule on the same workflow.
...
- **`license-check` (production tree, license-policy gate — non-blocking pilot)** — runs [`frontend/scripts/license-check.mjs`](./frontend/scripts/license-check.mjs), which scans the **production** dependency tree with `license-checker-rseidelsohn` and fails if any package's license is not on the allowlist in [`frontend/.supply-chain/license-allowlist.json`](./frontend/.supply-chain/license-allowlist.json). PARANOID posture: UNKNOWN / unlisted licenses fail closed — the only ways to resolve one are a `licenseOverrides` correction (a mis-declared-but-permissive license, set to the real SPDX confirmed from the LICENSE file), an exact-name `exemptions` entry, or a narrow publisher-controlled `exemptionPrefixes` entry; each carries a reason and a dated review. A scan-completeness guard fails the job rather than reporting a vacuous pass if the dependency-tree read comes back missing the repo's own direct production dependencies (a known `read-installed-packages` failure mode). The pure SPDX-evaluation logic in [`license-check.lib.mjs`](./frontend/scripts/license-check.lib.mjs) is unit-tested by `yarn license-check:test`, which the job runs pre-install (fail-fast, no `node_modules` needed) before the tree scan. The allowlist's only exemption today is a `@img/sharp-libvips-` prefix (LGPL-3.0 native libvips binary reached transitively via `next > sharp`; dynamically loaded, not statically combined). **Non-blocking pilot** — deliberately NOT in `all-checks-passed.needs`, so it reports without gating merges.
...
- **`all-checks-passed`** — single aggregator job that fails if any of the required CI-workflow gates above failed. The `license-check` pilot is intentionally excluded from its `needs:` while non-blocking (promote it per the TODO below). **Cross-workflow `needs:` is not supported**, so the gitleaks scan can't be a dependency of the aggregator. When branch protection is enabled, require both `All checks passed` and `Gitleaks / scan` as separate required contexts.
...
- [x] Add [`frontend/scripts/license-check.mjs`](./frontend/scripts/license-check.mjs) + [`license-check.lib.mjs`](./frontend/scripts/license-check.lib.mjs) + [`frontend/.supply-chain/license-allowlist.json`](./frontend/.supply-chain/license-allowlist.json) + the `license-check` job (non-blocking pilot) and `license-check:test` unit tests.
...
- [ ] **Promote `license-check` to a required gate** once it has run green for a week — add it to `all-checks-passed.needs` and require `License allowlist gate` as a branch-protection context. Until then it is an advisory pilot only.
